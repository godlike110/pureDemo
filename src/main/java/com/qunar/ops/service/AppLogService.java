...
public class AppLogService {
	 ...
	 */
	public Map<String, String> getBookingAndCreateLog(String site,String orderNo) throws ParseException, HttpException, IOException {
		...
		String price = infos.get("price");
		String fromDate = infos.get("fromDate");
		String type = infos.get("type");
		List<String> createLogList = new ArrayList<String>();
		if(type.equals("www")) {
		createLogList = shellService.execuShell("sh " + AptConstants.SHELL_PATH + "searchbcLog.sh create " + site + " " + date + " "  + timeh + " " + name + " " + fromDate + " " + date.split("-")[0] + " " + date.split("-")[1]);
		String deadLine = date + " " + time + ",999";
		String clog = getRightLog(createLogList, deadLine);
		result.put("clog", clog);
		List<String> bookingLog = shellService.execuShell("sh " + AptConstants.SHELL_PATH + "searchbcLog.sh booking " + site + " " + date + " "  + timeh + " " + ip+ " " + date.split("-")[0] + " " + date.split("-")[1]);
		if(bookingLog==null || bookingLog.size()==0 || StringUtils.isBlank(bookingLog.get(0))) {
			...
			bookingLog = shellService.execuShell("sh " + AptConstants.SHELL_PATH + "searchbcLog.sh booking " + site + " " + date + " "  + timeb + " " + ip+ " " + date.split("-")[0] + " " + date.split("-")[1]);
		}
		if(StringUtils.isNotBlank(bookingLog.get(0))) {
			result.put("blog", decretebLog(getRightLog(bookingLog,clog.split("\\[")[0])));
		}
		} else {
			createLogList = shellService.execuShell("sh " + AptConstants.SHELL_PATH + "searchbcLogwap.sh create " + site + " " + date + " "  + timeh + " " + name + " " + fromDate + " " + date.split("-")[0] + " " + date.split("-")[1]);
			String deadLine = date + " " + time + ",999";
			String clog = getRightLog(createLogList, deadLine);
			result.put("clog", clog);
			List<String> bookingLog = shellService.execuShell("sh " + AptConstants.SHELL_PATH + "searchbcLogwap.sh booking " + site + " " + date + " "  + timeh + " mobileUid=" + clog.split("mobileUid=")[1].split(",")[0]+ " " + date.split("-")[0] + " " + date.split("-")[1]);
			if(bookingLog==null || bookingLog.size()==0 || StringUtils.isBlank(bookingLog.get(0))) {
				String timeb = String.valueOf(Integer.parseInt(timeh)-1);
				bookingLog = shellService.execuShell("sh " + AptConstants.SHELL_PATH + "searchbcLogwap.sh booking " + site + " " + date + " "  + timeb + " mobileUid=" + clog.split("mobileUid=")[1].split(",")[0] + date.split("-")[0] + " " + date.split("-")[1]);
			}
			if(StringUtils.isNotBlank(bookingLog.get(0))) {
	...
	public Map<String, String> getPhoneCode(String site,String orderNo) throws ParseException {
		...
		Map<String, String> result = new HashMap<String, String>();
		List<String> tgqLogs = shellService.execuShell("sh " + AptConstants.SHELL_PATH + "getphonecodefromtgq.sh " + orderNo + " " + site);
	...
	private String decretebLog(String str) throws HttpException, IOException {
		...
		String decrytResponse = HttpUtil.getForObject("http://10.86.212.187:8080/flagshiptools/secret/simpleDecrypt?content=" + URLEncoder.encode(param) + "&_=" + System.currentTimeMillis());
